...
from analysis.utils.audio import plot_audio, read_audio
...
def audio_column(col):
    with col:
        ...
        with st.form("Prepare Input"):
            uploaded_file = st.file_uploader("Select your song 🎵", type=["wav", "mp3"])
        ...
        if read_data:
            ...
            st.session_state["audio"], st.session_state["sample_rate"] = read_audio(
                uploaded_file, sample_rate=sample_rate
            )

        if st.session_state["audio"] is not None:
            st.success(icon="🚀", body=f"Audio correcly loaded with shape {st.session_state['audio'].shape}")
            st.audio(st.session_state["audio"].T, sample_rate=st.session_state["sample_rate"])
